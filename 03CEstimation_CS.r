...
fun_plot_present <- function(data, overall.att, figlab) {
    f <- ggplot(data, aes(x = egt, y = att.gt)) + geom_point(pch = 1, size = 3) +  # nolint
        geom_errorbar(aes(ymin = lower, ymax = upper), width=.2) + 
        ylab("Effect by the length of exposure") + xlab("relative year") +
        ggtitle(figlab) +
...
file =  file.path(outpath, "CS_eventstudy.pdf"), height = 10, width = 6)
...
att_nyt1 <- store[[2]]
se_nyt1 <- store[[3]]
f0nnt <- fun_plot_present(store[[1]], store[[2]], figlab = "C. Baseline specification (nyt)")
ggsave(f0nnt, filename = file.path(outpath, "present_CS_es_01.pdf"), width = 10, height = 8)
...
ggsave(ggarrange(f0, f1, ncol = 1, nrow = 2),
file =  file.path(outpath, "CS02_eventstudy.pdf"), height = 10, width = 6)

ggsave(ggarrange(f0, f1, f0nnt, f1nnt,  ncol = 2, nrow = 2),
file =  file.path(outpath, "CS_eventstudy_combined.pdf"), height = 6, width = 6)
...
fatt0 <- ggplot(mat, aes(type, att)) + geom_point(pch = 1, size = 3) + 
geom_errorbar(aes(ymin = llim, ymax = ulim), width=.2) + 
theme_bw() + xlab("specification type") + ylab("overall average treatment effect \n on treated") + 
geom_hline(yintercept = -0.56, lty = "dashed", color = "red") + 
ggtitle("A. Overall ATT from CS (2004-2010)") + 
theme(text = element_text(size = 13)) + 
annotation_custom(grob)

ggsave(fatt0,
file =  file.path(outpath, "CS_overall2004to2010.pdf"), height = 6, width = 6)
